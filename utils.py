...
nlp_tgt = spacy.load("it_core_news_sm")

def segregate_to_sentence_level_src(input_text):
    # Use spacy to segment the input text into sentences
    sentences = [sentence.text.strip() for sentence in nlp_src(input_text).sents]
    return sentences

def segregate_to_sentence_level_tgt(input_text):
    # Use spacy to segment the input text into sentences
    sentences = [sentence.text.strip() for sentence in nlp_tgt(input_text).sents]
    return sentences
...
def get_ds(config):
    ...
    for i in range(len(ds_raw)):
        ...
        sentences_src = segregate_to_sentence_level_src(ds_raw[i]['translation'][config.get_config()['lang_src']])
        # print(sentences_src)
        sentences_tgt = segregate_to_sentence_level_tgt(ds_raw[i]['translation'][config.get_config()['lang_tgt']])
        # print(sentences_tgt)
